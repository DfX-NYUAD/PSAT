...
#PBS -e $stderr
cd $outdir
./sle -s -t -T $fifile -f 0.05 $infile -o ${base}_enc05.bench
./sle -s -t -T $fifile -f 0.10 $infile -o ${base}_enc10.bench
./sle -s -t -T $fifile -f 0.25 $infile -o ${base}_enc25.bench
./sle -s -t -T $fifile -f 0.50 $infile -o ${base}_enc50.bench
""")

# argument 1 bench dir
# argument 2 output dir
# argument 3 fault-impact-dir

def create_script(infile, outdir, fidir):
    basename = os.path.basename(infile)
    script_name = os.path.join(outdir, basename[:basename.find('.bench')] + '.sh')

    base = os.path.join(outdir, basename.replace('.bench', ''))
    fifile = os.path.join(fidir, basename.replace('.bench', '.faultimpact'))
...
for f in fs:
    create_script(f, sys.argv[2], sys.argv[3])
